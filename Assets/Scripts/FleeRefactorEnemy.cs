...
public class FleeRefactorEnemy : MonoBehaviour
{
    ...
    public struct Stats
    {
        ...
        [Tooltip("How fast the enemy runs away from the player (only when idle is false).")]
        public float runAwaySpeed;

        [Tooltip("Whether the enemy is idle or not. Once the player is within distance, idle will turn false and the enemy will run away from the player.")]
        public bool idle;
    ...
    private void Update()
    {
        // changes the enemy's behavior: pacing in circles or running away from the player
    ...
    private void RunAway()
    {
        ...
        transform.position = Vector3.MoveTowards(transform.position, player.transform.position, -Time.deltaTime * enemyStats.runAwaySpeed);
    }
    ...
    {
        // start running away if the player gets close enough
    ...
    {
        // stop running away if the player gets far enough away
        if (other.gameObject.tag == "Player")
        {
            enemyStats.idle = true;
        }
